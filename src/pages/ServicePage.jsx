  ...
  return (
    <div className="font-roboto overflow-x-hidden">
      ...
      <section className="relative h-[52vh] min-h-[380px] flex items-end">
        ...
        <div className="relative max-w-7xl mx-auto px-6 lg:px-10 pb-14 lg:pb-20 w-full">
          ...
          <div className="flex items-center gap-2 mb-5">
            <Link to="/" className="font-roboto text-xs tracking-[0.15em] uppercase text-white/60 hover:text-white/90 transition-colors">
              Home
            </Link>
            <span className="text-white/40 text-xs">/</span>
            <Link to="/services" className="font-roboto text-xs tracking-[0.15em] uppercase text-white/60 hover:text-white/90 transition-colors">
              Services
            </Link>
            <span className="text-white/40 text-xs">/</span>
            <span className="font-roboto text-xs tracking-[0.15em] uppercase text-white/90">{service.title}</span>
          </div>
          ...
          <h1 className="font-oswald text-4xl lg:text-6xl font-bold text-white uppercase leading-tight max-w-3xl">
            {service.title}
          </h1>
          <p className="font-roboto text-base lg:text-lg text-white/80 mt-4 max-w-xl">
            {service.tagline}
          </p>
        </div>
      </section>
      ...
      {service.gallery && service.gallery.length > 0 && (
        <section className="bg-[#F8F9FB] py-16 lg:py-20 border-t border-gray-100">
          <div className="max-w-7xl mx-auto px-6 lg:px-10">
            <div className="flex items-center gap-3 mb-10">
              <div className="w-8 h-px bg-secondary" />
              <span className="font-oswald text-[11px] tracking-[0.35em] uppercase text-primary font-medium">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 lg:gap-4">
              {service.gallery.map((img, i) => (
                <div
                  key={i}
                  className={`overflow-hidden ${i === 0 ? 'col-span-2 lg:col-span-2 row-span-1' : ''}`}
                >
                  <motion.img
                    initial={{ scale: 1.05 }}
                    whileInView={{ scale: 1 }}
                    ...
                    className={`w-full object-cover ${i === 0 ? 'h-72 lg:h-80' : 'h-52 lg:h-60'}`}
                  />
                </div>
              ))}
            </div>
